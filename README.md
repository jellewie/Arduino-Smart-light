...
## Hardware
- [PCB lamp](https://easyeda.com/jellewietsma/smart-light) or [PCB wall](https://easyeda.com/jellewietsma/smart-clock)
- All parts listed in the BOM of the PCB, these include resistors, capacitors, buttons, potmeters, and an ESP32
- Others things like Potmeter caps, DC jack cable and adapter, but also some magnets if you want the magnetic option for the desk version (the size of these are default 4x10mm round magnets)
## Firmware (To compile)
...
- If it was connected to WIFI it will show its IP, Which is explained in [WIFI page](#wi-fi-page) - Getting it’s IP.
...
The most up-to-date values can be find in the top of the ‘handler.ino’ source file. Some settings can only be set in [smart-light.local/IP](smart-light.local/IP), others can be done by the settings menu in the main screen.
...
5.	When the device is connected to WIFI it can be accessed by its IP address, but on devices who support mDNS, like Windows and Apple, [smart-light.local/info](smart-light.local/info) can be used.
