...
const char WiFiManager_EEPROM_Seperator = char(9);  //use 'TAB' as a seperator
...
char ssid[16] = "";       //Also defines howmany characters can be in the SSID
char password[16] = "";   //^
//#define strip_ip, gateway_ip, subnet_mask to use static IP
const int WiFiManager_EEPROM_SIZE = 255;   //Max Amount of chars of 'SSID + PASSWORD' (+1) (+extra custom vars)
...
bool WiFiManager_Set_Value(byte WiFiManager_ValueID, String WiFiManager_Temp) {
  ...
  switch (WiFiManager_ValueID) {
    ...
    case 3:
      ...
      break;
    case 4:
      HourlyAnimationS = WiFiManager_Temp.toInt();
      break;
    case 5:
...
String WiFiManager_Get_Value(byte WiFiManager_ValueID, bool WiFiManager_Safe, bool WiFiManager_Convert) {
  ...
  switch (WiFiManager_ValueID) {
    ...
    case 3:
      ...
      break;
    case 4:
      WiFiManager_Temp_Return = HourlyAnimationS;
      break;
    case 5:
      if (WiFiManager_Convert)
...
//==============================
void WiFiManager_StartServer() {                              //Start the webserver
...
void WiFiManager_EnableSetup(bool WiFiManager_TEMP_State) {   //Enable/disable setup page
#ifdef WiFiManager_SerialEnabled
...
byte WiFiManager_APMode() {                                   //Start a WIFI APmode
...
String WiFiManager_LoadEEPROM() {                             //Get raw data from EEPROM
...
byte WiFiManager_Start() {                                    //Start all WIFI stugg
